...
void as5600_init(as5600_t *as5600, i2c_port_t i2c_num, uint8_t scl, uint8_t sda, uint8_t out)
{
    ...
    if (!i2c_init(&as5600->i2c_handle, i2c_num, scl, sda, AS5600_I2C_FREQ_HZ, AS5600_SENSOR_ADDR)) {
...
float as5600_adc_get_angle(as5600_t *as5600)
{
    ...
    if (as5600->adc_handle.is_calibrated && as5600->conf.OUTS == AS5600_OUTPUT_STAGE_ANALOG_RR) {
        ...
        voltage = AS5600_LIMIT(voltage, AS5600_VCC_3V3_MIN_RR_MV, AS5600_VCC_3V3_MAX_RR_MV);
        angle = AS5600_MAP((float)voltage, AS5600_VCC_3V3_MIN_RR_MV, AS5600_VCC_3V3_MAX_RR_MV, 0, 360);
    }
    else {
        angle = -1;
    }
...
void as5600_init_adc_shared(as5600_t *as5600, adc_oneshot_unit_handle_t shared_handle)
{
    if (adc_config_channel(&as5600->adc_handle, as5600->out, shared_handle)) {
        ESP_LOGI(TAG, "AS5600 on GPIO %d initialized", as5600->out);
    } else {
        ESP_LOGE(TAG, "Failed to init AS5600 on GPIO %d", as5600->out);
    }
}
